...
for LED_scenario in LED_scenarios:
    ...
    while trial_cnt < 1:
        ...
        if plot_MarketShare_Total_Aggregated:
            plot_MS_Total_Aggregated(MarketShare_Total, years, LED_scenario)


        trial_cnt += 1


    #diagnostics dataframe for Scen_CarSegment
    d = Scen_CarSegments[Scen_CarSegments.LSOA == Scen_CarSegments.LSOA.tolist()[0]][['Year'] + [c for c in Scen_CarSegments.columns.tolist() if c.startswith('SP')]]
